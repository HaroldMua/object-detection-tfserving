...
class BaseCamera:
    ...
    def ip_thread(cls, unique_name):
        ...
        frames_iterator = cls.ip_frames(device)
        try:
            for cam_id, frame in frames_iterator:
                BaseCamera.frame[unique_name] = cam_id, frame
                BaseCamera.event[unique_name].set()  # send signal to clients
                ...
                if time.time() - BaseCamera.last_access[unique_name] > 60:
                    ...
                    # print('Closing camera_ip socket at port {}.'.format(port))
                    print('Stopping camera_ip thread for device {} due to inactivity.'.format(device))
                    pass
        except Exception as e:
            frames_iterator.close()
            # image_hub.zmq_socket.close()
